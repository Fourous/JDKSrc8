...
public class ChatServer {
    public static void main(String[] args) throws IOException {
        // 调用SelectorProvider.provider().openSelector() 通过平台选择合适的Selector
        ...
        while (true) {
            ...
            while (iterator.hasNext()) {
                ...
                if (selectionKey.isAcceptable()) {
                    ...
                    // 注意，这里是只进行了注册连接未发送消息，所以需要注册一个read事件
    ...
    private static class ChatHolder {
        ...
        private static void send(SocketChannel socketChannel, String msg) {
            try {
                // ByteBuffer 分配内存空间-固定
                ByteBuffer writeBuffer = ByteBuffer.allocate(1024);
                // buffer写入 channel read 方法也可
                writeBuffer.put(msg.getBytes());
                // flip 切换为读模式
                writeBuffer.flip();
                // channel 写入 buffer read 方法也可
